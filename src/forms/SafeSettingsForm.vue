...
const saveSafe = () => {
  safeStore.updateSafeName(newName.value, props.id);
  closeForm();
}

const deleteSafe = () => {
  if(confirm("Видалити рахунок " + props.name + "?")){
    safeStore.deleteSafe(props.id);
    closeForm();
  }
}
...
<div class="safe-settings-form" @click.self="closeForm">
  <div class="form-box">
    <form class="form" @submit.prevent="saveSafe">
      <h3 class="h3">Налаштування</h3>
      <div class="input-box">
        <label for="name">Назва</label>
        <input type="text" id="name" v-model="newName" required>
      </div>
      <button class="submit-btn" type="submit">Зберегти</button>
      <button class="submit-btn delete" type="button" @click="deleteSafe">Видалити</button>
    </form>
  </div>
</div>
</template>
